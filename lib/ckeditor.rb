  ...
  module Helper
    ...
    def ckeditor_textarea(object, field, options = {})
      ...
      id = if options[:id].nil?
		ckeditor_element_id(object, field)
	   else
		ckeditor_element_id(options[:id])
	   end
	...
	name_tag = if options[:name].nil?
      ...
      return inputs << javascript_tag("CKEDITOR.replace('#{id}', {
    ...
    def ckeditor_element_id(object, field = nil)
	if field.nil?
		"#{object}_editor"
	else
